...
class TSSAM(nn.Module):
    def __init__(self, image_encoder, num_class=8, in_channels=768, out_channels=256):
        ...
        self.t = ConvolutionalSideAdapter(256, 768, 256)

        # 创建CSA和MRM模块，每个ViT Block后一个
        self.csa_modules = nn.ModuleList([
            ConvolutionalSideAdapter(256, 768, 256) for _ in range(14)
        ])
        self.mrm_modules = nn.ModuleList([
            MultiScaleRefineModule(768, 256) for _ in range(12)
        ])
        self.mrm2 = MultiScaleRefineModule(256, 256)
        ...
        self.conv2 = nn.Conv2d(256, 768, kernel_size=3, padding=1)
        self.stage1_conv = nn.Conv2d(512, 256, kernel_size=3, padding=1)
        self.stage2_conv = nn.Conv2d(512, 256, kernel_size=3, padding=1)
        self.final_conv = nn.Conv2d(256, num_class, kernel_size=1)  # 最终输出num_class通道的掩码
    ...
    def forward(self, x):
        ...
        F_stage4_csa = F.interpolate(F_stage4_csa, size=(origin_x.size(2), origin_x.size(3)), mode='bilinear',
                                     align_corners=False)
        # [B, C, H, W]
...
if __name__ == "__main__":
    ...
    print(f"Model parameters occupy {model_size_in_MB:.2f} MB of memory.")

    # 计算单个 batch 的输入数据占用的显存
    input_size_in_bytes = batch_size * 3 * 1024 * 1024 * 4  # 4 字节是 float32 的大小
    input_size_in_MB = input_size_in_bytes / (1024 ** 2)
    print(f"One batch of input data occupies {input_size_in_MB:.2f} MB of memory.")
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, masks) in enumerate(train_loader):
            ...
            # print(f"Unique values in mask {i}: {torch.unique(masks)}")
            # 应该只输出 0 到 20 之间的值
